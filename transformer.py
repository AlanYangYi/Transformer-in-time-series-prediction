...
def make_data():
    ...
    df2 = df.groupby('area_id').apply(create_count_feature)
    return df2
...
def get_attn_subsequence_mask(seq):
    ...
    subsequence_mask = np.triu(np.zeros(attn_shape), k=1)
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Linear(input_features_for_encoder, d_model)
    ...
    def forward(self, enc_inputs):
        ...
        """
        enc_outputs = self.src_emb(enc_inputs.unsqueeze(-1))  # [batch_size, src_len, d_model]
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.tgt_emb = nn.Linear(input_features_for_decoder, d_model)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        """
        dec_outputs = self.tgt_emb(dec_inputs.unsqueeze(-1))
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.decoder2=nn.Linear(512*30,7).to(device)
        # self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False).to(device)
        self.projection = nn.Linear(d_model, projection_feature).to(device)
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        dec_outputs=self.decoder2(enc_outputs.view(-1,512*30))
        # dec_outputs: [batch_size, tgt_len, d_model] -> dec_logits: [batch_size,tgt_len, output_features(projection_features)]
        #dec_logits = self.projection(dec_outputs)



        return dec_outputs.view(-1), enc_self_attns, dec_self_attns, dec_enc_attns
...
def test_process_autoregrssive(test_loader, model):
    print("********prediction**********")
    ...
    for enc_inputs_, dec_inputs_, dec_outputs_ in test_loader_:
        ...
        all_loss_=all_loss_+loss.item()
        k=k+1.0

    return (sum.data.cpu().numpy() / number,all_loss_.cpu().numpy()/k)


def test_process(test_loader, model_):
    print("********prediction**********")
    ...
    for enc_inputs_, dec_inputs_, dec_outputs_ in test_loader_:
        k=k+1.0
        model_.eval()
        #model_.apply(apply_dropout)
        model_.to(device)
        enc_inputs_, dec_inputs_, dec_outputs_ = enc_inputs_.to(device), dec_inputs_.to(device), dec_outputs_.to(device)
        with torch.no_grad():
            outputs_, enc_self_attns_, dec_self_attns_, dec_enc_attns_ = model_(enc_inputs_, dec_inputs_)
    ...
    return (sum.cpu().numpy() / number, all_loss_/ k)
...
#         enc_inputs_, dec_inputs_, dec_outputs_ = enc_inputs_.to(device), dec_inputs_.to(device), dec_outputs_.to(device)
#         with torch.no_grad():
#             outputs_, enc_self_attns_, dec_self_attns_, dec_enc_attns_= model(enc_inputs_, dec_inputs_)
#             loss_ = criterion(outputs_,
#                               dec_outputs_.view(-1))  # dec_outputs.view(-1):[batch_size * tgt_len * tgt_vocab_size]
#             all_loss_ += loss_.item()
#         number = number + outputs_.size().numel()
...
data = make_data()
training_set = data[('2022-03' == data.DATE )]
test_set = data[data.DATE >= '2022-04']

enc_inputs = torch.from_numpy(training_set.iloc[:, 10:].to_numpy())
dec_inputs = torch.from_numpy(training_set.iloc[:, [10, 11, 12, 13, 14, 15, 16]].to_numpy())
#dec_inputs = torch.from_numpy(training_set.iloc[:, [10, 9, 8, 7, 6, 5, 4]].to_numpy())
dec_outputs = torch.from_numpy(training_set.iloc[:, [9, 8, 7, 6, 5, 4, 3]].to_numpy())
...
loader = Data.DataLoader(MyDataSet(enc_inputs, dec_inputs, dec_outputs), 300, True)



enc_inputs_ = torch.from_numpy(test_set.iloc[:, 10:].to_numpy())
dec_inputs_ = torch.from_numpy(test_set.iloc[:, [10, 11, 12, 13, 14, 15, 16]].to_numpy())
#dec_inputs_ = torch.from_numpy(test_set.iloc[:, [10, 9, 8, 7, 6, 5, 4]].to_numpy())
dec_outputs_ = torch.from_numpy(test_set.iloc[:, [9, 8, 7, 6, 5, 4, 3]].to_numpy())
...
criterion = nn.L1Loss()
#optimizer = optim.SGD(model.parameters(), lr=0.1,weight_decay=1e-4,momentum=0.99)
optimizer = optim.SGD(model.parameters(), lr=5)

# lr = 0.5  # learning rate
# optimizer = torch.optim.SGD(model.parameters(), lr=lr,weight_decay=1e-4)
# scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer, 1.0, gamma=0.95,verbose=True)
...
scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer, 2.0, gamma=0.95,verbose=True)
#scheduler2=torch.optim.lr_scheduler.StepLR(optimizer, 20, gamma=0.95,verbose=True)

mapes,test_losses,traning_losses=[],[],[]

# dics=torch.load("transformer_params.pkl")
# model.load_state_dict(dics)
...
for epoch in range(epochs):
    ...
    #     scheduler1.step()
    # else:
    #     if scheduler2.get_last_lr()[0] > 0.005:
    #         scheduler2.step()
    #     if all_loss/i < 10:
    #         scheduler2.step()
    ...
    #     scheduler1.step()
    # if epoch >50 and scheduler2.get_last_lr()[0] > 0.001:
    #     scheduler2.step()
